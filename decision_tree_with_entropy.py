...
def findfeaturethreshold(array):
    ...
    for feature in range(cols-1):
        ...
        for threshold in thresholds:
            ...
            weightedsumgimp = (leftone.shape[0]*entropy(leftone[:, -1]) + rightone.shape[0]*entropy(rightone[:,-1]))/rows
...
# define a recursive helper function called growtree
    def growtree(self, xtrain, ytrain, depth = 0):
        if depth == self.maxdepth or entropy(ytrain) == 0 or xtrain.shape[0]<=self.minsamples:
...
# Define the print_tree function with feature names
def print_tree(node, depth=0):
    if node.is_leaf_node():
        # Use label_mapping to display the class label (e.g., Beer, Wine, Whiskey)
        print(f"{'|   ' * depth}Leaf: {label_mapping[int(node.value)]}")
    else:
        # Use feature_names to display the feature name instead of its index
